...
const API_BASE = "/apis/api.plugin.halo.run/v1alpha1/astrahub/world-chat";
...
async function request<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  const payload = await parseJson<T & WorldChatErrorPayload>(response);
  if (!response.ok) {
    throw new Error(buildWorldChatErrorMessage(response, payload));
  }
  return payload;
}

export function hasWorldChatAccess(settings: AstraHubSettings) {
  return Boolean(
    String(settings.credentials.siteId || "").trim() &&
    String(settings.credentials.apiKey || "").trim()
  );
}

export async function fetchWorldChatBootstrap() {
  return request<WorldChatBootstrapResponse>("/bootstrap");
}
...
export async function markWorldChatRead() {
...
export function worldChatStickerFileUrl(stickerId: string) {
  return `${API_BASE}/stickers/${encodeURIComponent(stickerId)}/file`;
}
