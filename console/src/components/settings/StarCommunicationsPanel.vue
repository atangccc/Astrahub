...
const canAccess = computed(() => hasWorldChatAccess(props.settings));
...
async function loadBootstrap() {
  ...
  try {
    ...
    if (bootstrap.member?.siteId) {
      upsertMembers([bootstrap.member]);
    }
    await loadStickers();
...
function avatarUrl(member?: { avatarUrl?: string } | null) {
  ...
  if (raw.startsWith("/")) {
    ...
    return hubBaseUrl ? `${hubBaseUrl}${raw}` : raw;
  }
  return raw;
}
...
.sc-avatar img,.sc-member-avatar img,.sc-profile-avatar img{width:100%;height:100%;object-fit:cover}
