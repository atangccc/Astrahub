...
public class AstraHubLinkEdgePushService {

    private static final ObjectMapper MAPPER = new ObjectMapper();
    private static final String PUSH_PATH = "/v1/site-link-edges/push";
    ...
    public Mono<PushResult> push() {
        return exportService.export()
            .flatMap(payload -> pushPayload(payload));
    }

    public Mono<PushResult> push(ServerRequest request) {
        return exportService.export(request)
            .flatMap(payload -> pushPayload(payload));
    }

    public Mono<PushResult> push(AstraHubLinkEdgeExportService.LinkEdgesPayload payload) {
    ...
    private PushResult pushBlocking(
        AstraHubLinkEdgeExportService.LinkEdgesPayload payload,
        JsonNode connection,
        ...
        try {
            String endpoint = hubBaseUrl + PUSH_PATH;
            String body = MAPPER.writeValueAsString(payload);
            SignedRequest signed = HubRequestSigner.signRequest("POST", PUSH_PATH, body, siteId, apiKey);
            ...
            String message = success ? "site link edges push success" : extractErrorMessage(responseBody, status);

            return new PushResult(success, status, message, shrink(responseBody, 2000), nowIso());
        } catch (Exception error) {
            log.warn("[AstraHub] link edge push request failed", error);
    ...
    private static String extractErrorMessage(String responseBody, int status) {
        try {
            ...
            if (error != null && error.isObject()) {
                message = trim(error.path("message").asText(""));
                if (!message.isEmpty()) {
                    return message;
                }
            }
